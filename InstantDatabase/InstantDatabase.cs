...
namespace Xamarin.Data
{
	public class InstantDatabase : SQLiteConnection
	{
		...
		public void MakeClassInstant (Type type)
		{
			...
			//grab group data and store it for quick retreval
		...
		private void FillGroups(Type type)
		{
			...
			lock(DatabaseLocker)
				groups = this.Table<InstantDatabaseGroup> ().Where (x => x.ClassName == type.Name).OrderBy(x=> x.Order).ToList ();
		...
		private void SetGroup(Type t)
		{
			List<InstantDatabaseGroup> groups;
			lock(DatabaseLocker)
				groups = this.Table<InstantDatabaseGroup> ().Where (x => x.ClassName == t.Name).OrderBy (x => x.Order).ToList ();
			if (!Groups.ContainsKey (t))
				Groups.Add (t, groups);
			else
				Groups [t] = groups;
